///////////////////////////////////////////////////////////////////  
/*                    GPS-Decoder w. OLED SH1306                 */
///////////////////////////////////////////////////////////////////
/*  MCU:              STM32F4 (ARM Cortex M4)                    */
/*  Hardware:         F411 Blackpill board                       */
...
//Connections
//PA3: Data in from GPS module (TX port)
//PB6: OLED SCK
//PB9: OLED SDA
...
{	
	...
	char *pointer0; //Variable for determining if "$GPGLL" string is in input buffer
	int sx = 0, sy = 2;
	
	msg = (char*)malloc(MAXBUFFER + 1);
    ...
    oled_putstring(0, 0, (char*)"DK7IH GPS Decoder", 0, 0); 
    
    while(1)
    {
        
        if(scomplete) //Transmission completed -> process string
        {
			pointer0 = strstr(msg, "$GPGLL"); //Find NMEA string identifier in input buffer
			if(pointer0)
			{
				sx = 0; //OLED coordinates for next char on screen
				sy = 2;
			    oled_cls(0);
			    oled_putstring(0, 0, (char*)"DK7IH GPS Decoder", 0, 0); 
				for(t1 = 0; msg[t1] && t1 < 160; t1++) //Display string
			    {
				    if((msg[t1] > 31) && (msg[t1] < 127))
				    {
				        oled_putchar1(sx * FONTW, sy, msg[t1], 0);
				    }
			...
			scomplete = 0;
			bufcnt = 0;
			for(t1 = 0; t1 < MAXBUFFER; t1++) //Re-init buffer
		    {
			    msg[t1] = 0;
		    }		
		}
